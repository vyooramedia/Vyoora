...
# --- OCR import flag only (no heavy init yet) ---
try:
    import easyocr
    OCR_IMPORT_OK = True
    OCR_IMPORT_ERR = ""
...
# -------- Lazy EasyOCR (avoid slow cold-starts) --------
_reader = None
_READER_ERR = None
def get_reader():
    global _reader, _READER_ERR
    if _reader is None and OCR_IMPORT_OK:
        try:
            _reader = easyocr.Reader(["en"], gpu=False)
            print("✅ EasyOCR ready")
        except Exception as e:
            _READER_ERR = f"EasyOCR init failed: {e}"
            print("❌", _READER_ERR)
    return _reader
...
def normalize_token(tok: str) -> str:
    ...
    if not tok or len(tok) < 2: return ""
    words = []
    for w in tok.split():
        words.append(w if re.fullmatch(r"[A-Z0-9\-(),/]+", w) else w.capitalize())
...
CANON = {
    "Aqua": "Water","Aqua/Water": "Water","Parfum": "Fragrance","Parfum/Fragrance": "Fragrance",
    "Alcohol Denat": "Alcohol Denat.","Tocopherol Acetate": "Tocopheryl Acetate",
}
def canonicalize(name: str) -> str: return CANON.get(name, name)

LOCAL_RULES = {
    "Water": ("Safe","Solvent/base in most formulas."), "Aqua": ("Safe","Solvent/base in most formulas."),
...
def cache_get(n:str): k=n.lower().strip(); 
# NOTE: one-liners to keep concise
    # noqa
...
def ai_clean_and_classify(ingredients: List[str]) -> List[Dict[str, Any]]:
    ...
    results: List[Dict[str, Any]] = []; to_ai: List[str] = []
    for ing in ingredients:
        if ing in LOCAL_RULES:
            cls, rsn = LOCAL_RULES[ing]; cache_put(ing, cls, rsn)
            results.append({"ingredient": ing, "classification": cls, "reason": rsn}); continue
        cached = cache_get(ing)
        if cached:
            cls, rsn = cached; results.append({"ingredient": ing, "classification": cls, "reason": rsn}); continue
        to_ai.append(ing)

    if not to_ai:
        order = {n:i for i,n in enumerate(ingredients)}
        results.sort(key=lambda x: order.get(x["ingredient"], 10**6)); return results
    ...
    schema = {
        "type":"object","properties":{"items":{"type":"array","items":{
            "type":"object","properties":{
                ...
                "reason":{"type":"string"}},
            "required":["ingredient","classification","reason"],"additionalProperties":False}}},
        "required":["items"],"additionalProperties":False}

    sys = ("You are Vyoora, a concise skincare ingredient expert. "
    ...
    try:
        resp = oai_client.chat.completions.create(
            ...
            response_format={"type":"json_schema","json_schema":{"name":"ingredient_classifications","schema":schema,"strict":True}}
        )
        import json
        data = resp.choices[0].message.content
        items = (data if isinstance(data, dict) else json.loads(data)).get("items", [])
        for it in items:
            ing = str(it.get("ingredient","")).strip()
            if not ing: continue
            cls = str(it.get("classification","Limited Data")).strip()
            if cls not in {"Safe","Irritant","Comedogenic","Limited Data"}: cls="Limited Data"
        ...
        for ing in to_ai:
            ...
            cache_put(ing, cls, rsn); results.append({"ingredient": ing, "classification": cls, "reason": rsn})
...
@app.get("/")
def root():
    return jsonify({"ok": True, "service": "vyoora-backend"})
...
def upload():
    r = get_reader()
    if not OCR_IMPORT_OK or r is None:
        return jsonify({"error": f"OCR unavailable: {_READER_ERR or OCR_IMPORT_ERR}"}), 500
    ...
    try:
        lines_line = r.readtext(tmp_path, detail=0, paragraph=False)
        lines_para = r.readtext(tmp_path, detail=0, paragraph=True)
...
def chat():
    ...
    try:
        completion = oai_client.chat.completions.create(
            ...
            messages=[
                {"role":"system","content":"You are Vyoora, a concise skincare assistant. Be practical, avoid medical claims, and suggest patch testing."},
